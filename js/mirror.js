...
var cardtemplate = "assets/images/gothic_mirror_frame.png";
...
const fragPlane = `
  ...
  void main() {
    ...
    // Procedural Border Logic
    float borderThickness = 0.03; // Thin luxurious frame
    float glowSoftness = 0.02;     // Glow width

    // Distance from edge
    vec2 dist = min(vUv, 1.0 - vUv);
    float edgeDist = min(dist.x, dist.y);

    // Purple Color (Neon/Cyber)
    vec3 borderCol = vec3(0.77, 0.1, 0.96); // Vivid Purple
    
    if (edgeDist < borderThickness) {
        // Solid Border
        gl_FragColor = vec4(borderCol, 1.0);
        // Add metallic shine
        gl_FragColor += vec4(f * 0.5); 
    } else if (edgeDist < borderThickness + glowSoftness) {
        // Inner Glow
        float alpha = 1.0 - ((edgeDist - borderThickness) / glowSoftness);
        gl_FragColor = vec4(borderCol * alpha, 1.0); // Fade into black
        gl_FragColor.rgb += vec3(f * 0.2); // Subtle reflection
    } else {
        // Center: Black Screen
        gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); // Opaque Black
        
        // Very subtle glass reflection (optional, keeps it feeling solid)
        gl_FragColor += vec4(f * 0.15); 
    }
  }
`;
...
function plane() {
  var geometry = new THREE.PlaneGeometry(22, 32);
...
function updateDraw(deltaTime) {
  ...
  controls.autoRotate = options.isanimate;
  controls.update();
...
function updateThemeColors() {
  ...
  const target = isLight ? themes.light : themes.dark;

  options.color0 = target.color0;
  options.color1 = target.color1;
  options.color2 = target.color2;
}
...
function handleResize() {
  ...
  camera.aspect = w / (window.innerHeight || container.clientHeight);
...
const cameraToggle = document.getElementById('cameraParams');
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    navigator.mediaDevices.getUserMedia({ video: true })
      .then((stream) => {
        ...
        if (frontcard) {
          originalMaterial = frontcard.material;
          frontcard.material = new THREE.MeshBasicMaterial({
            map: videoTexture,
            color: 0xffffff,
            reflectivity: 1
          });
...
function stopWebcam() {
  ...
  if (frontcard && originalMaterial) {
    frontcard.material = originalMaterial;
  }
}
